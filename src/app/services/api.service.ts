...
export class ApiService {
  ...
  getBlog(blogId: number): Observable<Blog> {
    return this.http.get<Blog>(
      'https://mi-blogs.azurewebsites.net/api/Blogs/' + blogId
    );
  }

  postBlog(blog: Blog): void {
    this.http
      .post<Blog>('https://mi-blogs.azurewebsites.net/api/Blogs/', blog)
      .subscribe((blog) => {
  ...
  editBlog(blog: Blog): Observable<Blog> {
    return this.http.put<Blog>(
      'https://mi-blogs.azurewebsites.net/api/Blogs/' + blog.id,
      blog
    );
  }
}
